...
function getSupabaseClient() {
  const url = window.COUNTRYSCORE_CONFIG.SUPABASE_URL || "";
  const key = window.COUNTRYSCORE_CONFIG.SUPABASE_KEY || "";
  ...
  if (supabaseClient) {
    try {
      const { data, error } = await supabaseClient
        .from("reviews")
        .select("id, created_at, country, status, category, subcategory, score, display_name, title, comment, approved")
        .eq("approved", true)
        .order("created_at", { ascending: false });

      if (error) throw error;

      remoteReviews = (data || []).map(row => ({
        id: row.id,
      ...
      setConnectionStatus("Live Supabase connected");
    } catch (error) {
      console.warn("Supabase load failed:", error);
      setConnectionStatus("Supabase offline - showing sample/local data");
    }
  } else {
    setConnectionStatus("Demo mode - add Supabase config");
  }

  reviews = dedupeReviews([...SAMPLE_REVIEWS, ...remoteReviews, ...localReviews].map(normalizeReview).filter(Boolean));
...
function renderSharedLayout() {
  ...
  navHost.innerHTML = `
    ...
    <div id="pageLoader" class="page-loader" hidden>
      ...
      <span>Loading CountryScore...</span>
    </div>
  `;
}
...
async function submitReview(event) {
  ...
  const cooldownError = getCooldownError(data.country, data.category);
  if (cooldownError) {
    msg.textContent = cooldownError;
    msg.className = "warning";
    return;
  }
  ...
  const duplicate = findDuplicateReview(review, reviews);
  if (duplicate) {
    msg.textContent = "This looks like a duplicate review. The original will be kept instead.";
    msg.className = "warning";
    return;
  }

  isSubmittingReview = true;
  setSubmitLoading(submitButton, true, "Posting...");
  ...
  try {
    if (supabaseClient) {
      const { error } = await supabaseClient
        ...
        .insert({
          ...
          score: review.score,
          display_name: review.displayName || "Anonymous",
          title: review.title,
      ...
      msg.textContent = "Review posted to Supabase. It should appear after the next live refresh.";
      msg.className = "success";
    } else {
      saveLocalReview(review);
      msg.textContent = "Saved locally for testing. Add Supabase config to write live.";
      msg.className = "success";
    }

    saveCooldown(review.country, review.category);
    reviews = dedupeReviews([...reviews, review]);
    form.reset();
    ...
    saveCooldown(review.country, review.category);
    reviews = dedupeReviews([...reviews, review]);
    msg.textContent = "Supabase submission failed, so it was saved locally for testing.";
    msg.className = "warning";
  } finally {
    isSubmittingReview = false;
    setSubmitLoading(submitButton, false, originalButtonText || "Submit review");
  }
}
...
function setSubmitLoading(button, isLoading, text) {
  if (!button) return;
  button.disabled = isLoading;
  button.classList.toggle("button-loading", isLoading);
  button.innerHTML = isLoading ? `<span class="spinner small"></span>${text}` : text;
}
...
function validateReviewInput(data) {
  ...
  if (title.length < MIN_TITLE_LENGTH) {
    return `Review title must be at least ${MIN_TITLE_LENGTH} characters.`;
  }

  if (comment.length < MIN_COMMENT_LENGTH) {
    return `Review text must be at least ${MIN_COMMENT_LENGTH} characters so it gives useful context.`;
  }

  if (!hasEnoughRealText(comment)) {
    return "Review text needs real words, not just punctuation, emojis, or repeated symbols.";
  }

  if (!hasEnoughRealText(title)) {
    return "Review title needs real words, not just punctuation or repeated symbols.";
  }

  return "";
}

function hasEnoughRealText(value) {
  const lettersAndNumbers = String(value).match(/[A-Za-zÀ-ÖØ-öø-ÿ0-9]/g) || [];
  const words = String(value).match(/[A-Za-zÀ-ÖØ-öø-ÿ0-9]{3,}/g) || [];
  const uniqueChars = new Set(String(value).toLowerCase().replace(/\s/g, "").split(""));
  return lettersAndNumbers.length >= 20 && words.length >= 5 && uniqueChars.size >= 8;
}

function getCooldownKey(country, category) {
  return `cs_cooldown_${slug(country)}_${slug(category)}`;
}

function getCooldownError(country, category) {
  const key = getCooldownKey(country, category);
  const lastTime = Number(localStorage.getItem(key) || 0);
  const remaining = REVIEW_COOLDOWN_MS - (Date.now() - lastTime);

  if (remaining <= 0) return "";

  const minutes = Math.ceil(remaining / 60000);
  return `Please wait ${minutes} more minute${minutes === 1 ? "" : "s"} before reviewing this same category in this same country again.`;
}

function saveCooldown(country, category) {
  localStorage.setItem(getCooldownKey(country, category), String(Date.now()));
}
...
function findDuplicateReview(review, existingReviews) {
  return existingReviews.find(existing => duplicateKey(existing) === duplicateKey(review));
}

function duplicateKey(review) {
  return [
    slug(review.country),
    slug(review.status),
    slug(review.category),
    slug(review.subcategory || "general"),
    String(Number(review.score).toFixed(1)),
    slug(review.title),
    slug(review.comment)
  ].join("|");
}
...
function saveLocalReview(review) {
  ...
  local.push(review);
  localStorage.setItem("cs_local_reviews", JSON.stringify(dedupeReviews(local)));
}
